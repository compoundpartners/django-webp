...
WEBP_ENABLED = getattr(settings, 'WEBP_ENABLED', True)
...
def _check_by_user_agent(user_agent):
    ...
    if user_agent:
        ...
        if 'browser' in data:
            return WEBP_ENABLED and data['browser']['name'] in WEBP_VALID_BROWSERS

    return False


def _check_by_http_accept_header(http_accept):
    return WEBP_ENABLED and 'webp' in http_accept
